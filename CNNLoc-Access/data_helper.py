...
def load_test_data(valid_file_name):
    ...
    testing_x = test_data_frame.get_values().T[:WAP_SIZE].T
    testing_y = test_data_frame.get_values().T[[301, 302, 303, 304], :].T
...
def load_data_perspective(file_name):
    ...
    data_y = data_frame.get_values().T[[301, 302, 303, 304], :].T
...
def load_grouped_data(trainingData):
    ...
    data_x = data.get_values().T[4:304].T
...
def load(train_file_name, valid_file_name):
    ...
    training_x = train_data.get_values().T[:301].T
    training_y = train_data.get_values().T[[301, 302, 303, 304], :].T
    validation_x = valid_data_trame.get_values().T[:301].T
    validation_y = valid_data_trame.get_values().T[[301, 302, 303, 304], :].T
    testing_x = test_data_frame.get_values().T[:301].T
    testing_y = test_data_frame.get_values().T[[301, 302, 303, 304], :].T
...
def normalizeX(arr,b=2.71828):
    ...
    return normalizeX_powed(arr,b).reshape([-1, 301])
